...
narrower — they execute bounded tasks in isolated worktrees. Your leverage is
...
2. **Repo safety preflight.** Confirm the target is a git repo. Run
   `git -C <repo> status --short` — clean, or record pre-existing dirt. Never run
   workers on `main`/`master` directly.
...
6. **Pick the model per worker via `loe` (luna-first).** You are the expensive
   ...
   *after* your decomposition — not the raw size of the feature:
   - **`loe: "low"` → `gpt-5.6-luna` at high reasoning.** The default you should
     reach for: a small model thinking hard. Use it whenever the goal is simple,
     mechanical, or you have already supplied a clear, well-specified plan a
     small model can just follow (boilerplate, tests from a spec, a localized
     edit, a rename, wiring described step by step). Reasoning tokens at nano
     pricing are nearly free; the effort bump derisks the tier most workers
     should run at.
   ...
   **Backend.** Default to `codex`. Pick `backend: "claude"` when the task
   ...
   worker's changes on its branch after the driver finishes (reliable — it runs
   outside the sandbox); with `commit: false` it leaves them in the worktree for
   you to review and commit. You review the diff before merging regardless.
...
8. **Synthesize — do NOT concatenate worker outputs.**
   ...
   - **Independently verify**: re-read changed source and run the tests yourself.
     A worker's self-reported success is never trusted (workers lack project
     context and will overstate or invent). After verifying each worker branch,
     ...
     `<task>-a<n+1>` per the rejected-diff row — it consumes retry budget. The
   ...
   - Autoreview the integrated result. For a **trivial** integrated diff, review
     ...
     **non-trivial** diff, run the **Review→fix loop** below instead of reviewing
     ...
     dependency or lockfile changes, new network surface, credential/auth
     surface, CI or global config, or a diff touching more than 8 files or 400
     changed lines.
...
**reviewer worker** is a force-multiplying first pass — never an authority. The
bedrock rule still holds: a reviewer is a worker too, so **you adjudicate every
finding against the real code** before any fix is dispatched. The reviewer cannot
...
- **Topology: post-integration.** Review the merged `integration/<project>` diff
  vs the recorded base — what actually ships — so one frontier pass also catches
  cross-worker integration bugs. Per-branch review is the exception, only for an
  individually large or risky branch.
...
1. Dispatch ONE reviewer (`templates/worker-goal.md` → **Reviewer worker**),
   ...
   credentials, injection, unexpected dependency/network additions — a
...
2. **Record** the reviewer's findings (from its final message in
   ...
   can't write the run dir — you do.
...
5. Else dispatch ONE **fix worker** (`templates/worker-goal.md` → **Fix worker**)
   ...
   findings only, `commit: true`. Then verify it yourself — run the tests and
   review the fix diff — and merge it with `integrate_branch`. Any fix that needs
   **git itself** — untracking already-committed build artifacts
   (`git rm --cached`), reverting a file — you do directly: workers never run
   git, so a worker can add a `.gitignore` but cannot un-commit what is already
   tracked.
...
- **Severity on CONFIRM.** Every confirmed finding carries P1|P2|P3. P1
  ...
  re-verified — security-flavored P1 stays a hard abort. P2 dispatches a fix
...
Calibration exemplar — how five typical verdicts should read:

| finding (abridged) | verdict | why |
| --- | --- | --- |
| "the merge guard ignores untracked files — bug" | REJECT | Recorded invariant: test artifacts must not block merges; git still refuses to clobber. Add the invariant to the next reviewer goal. |
...
| "error handling could be more robust" (no location) | REJECT | No file:line, no failure scenario — not actionable as written. |
...
model. Timeout never LOE-bumps — higher reasoning is slower. Vacuous
...
- Use fresh-dir isolation and never forced deletion. A stuck path is listed for
  the human; do not delete it harder.
...
`timeoutS` (default 1800, snapshotted per run at init); a timed-out worker is
killed with its process tree and lands as `failed` with `reason: "timeout"`. The
...
Competition is the retry budget spent in parallel: opt-in per task, flagged in
the plan doc with a recorded one-line reason. Never use it for mechanical,
fully-dictated slices.

Use exactly two competitors, named `<task>-c1` and `<task>-c2`, with the same
goal file. Dispatch both with `task: "<task>"` so each writes one ledger row: two
of the three dispatches, with one escalation rung in reserve. Launch `<task>-c1`
with `attempt: 1`, confirm its ledger row exists, then launch `<task>-c2` with
`attempt: 2` — the engine's attempt guard is sequential, so the launches stagger
even though the workers run concurrently. Cross-family is the default;
same-family competition needs a recorded reason. Both start at the task's start
rung. Use the policy string `compete:2 <fam>+<fam>`.
...
The winner merges via `integrate_branch`; the loser is archived by the close
gate. Salvaging a loser's better tests is a normal fix worker with the loser's
patch quoted in the goal.
